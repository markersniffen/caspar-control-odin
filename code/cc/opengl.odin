...
OpenglInit :: proc(Show: ^show)
{
	...
	gl.GenBuffers(1, &Show.State.glState.TextureVertexBuffer)
	gl.GenBuffers(1, &Show.State.glState.TextureIndexBuffer)
...
OpenglRender :: proc(Show: ^show)
{
	...
	OpenglRenderTextureRect(Show)
	glfw.SwapBuffers(glState.Window)
}

// Junk
// OpenglDrawFilledRect :: proc(Pos: v2, Size: v2, Color: v4, Center: bool, Space: v2i, Show: ^show)
// {
// 	glState := Show.State.glState;
...
// 	L := f32((Pos.x - W - Cx) / W)
// 	R := f32((Pos.x - W - Cx + Size.x) / W)
// 	T := f32((Pos.y - H - Cy + Size.y) / H)
// 	B := f32((Pos.y - H - Cy) / H)
	
// 	Vertices :[12]f32 = { L, T, 0, R, T,  0, R, B, 0, L, B, 0 };
// 	Indices :[6]u32 = { 0,1,2, 2,3,0 };
	
// 	gl.UseProgram(glState.flat_shader);
	
// 	glColor := gl.GetUniformLocation(glState.flat_shader, "color");
// 	gl.Uniform4f(glColor, f32(Color[0]), f32(Color[1]), f32(Color[2]), f32(Color[3]));
...
OpenglRenderTextureRect :: proc(Show: ^show)
{
	...
	gl.BindBuffer(gl.ARRAY_BUFFER, Show.State.glState.TextureVertexBuffer);
	gl.BufferData(gl.ARRAY_BUFFER, size_of(Vertices), &Vertices[0], gl.STATIC_DRAW);
	...
	gl.EnableVertexAttribArray(0)
	// gl.EnableVertexAttribArray(1)
	// gl.VertexAttribPointer(1, 2, gl.FLOAT, gl.FALSE, size_of([5]f32), uintptr(size_of([3]f32)))

	// bind vertex/UV data to buffer
	gl.BindBuffer(gl.ELEMENT_ARRAY_BUFFER, Show.State.glState.TextureIndexBuffer);
	gl.BufferData(gl.ELEMENT_ARRAY_BUFFER, 6 * size_of(f32), &Indices[0], gl.DYNAMIC_DRAW);
